--Fix for crash when using "packrat" weapon hold with auto fire

local orig_PlayerStandard__check_action_primary_attack = PlayerStandard._check_action_primary_attack
function PlayerStandard:_check_action_primary_attack(t, input)
	--Use original function if not Bernetti Auto or Czech 92
	local name_id = self._equipped_unit:base():get_name_id()
	if name_id ~= "beer" and name_id ~= "czech" then
		return orig_PlayerStandard__check_action_primary_attack(self, t, input)
	end
		...
		if not action_forbidden then
			...
			if self._equipped_unit then
				...
				elseif self._running and not self._equipped_unit:base():run_and_shoot_allowed() then
					...
					if fired then
						--Play single-fire recoil animation instead
						if fire_mode == "auto" then
...
local orig_PlayerStandard__check_stop_shooting = PlayerStandard._check_stop_shooting
function PlayerStandard:_check_stop_shooting()
	--Use original function if not Bernetti Auto or Czech 92
	local name_id = self._equipped_unit:base():get_name_id()
	if name_id ~= "beer" and name_id ~= "czech" then
		return orig_PlayerStandard__check_stop_shooting(self)
	end
