...
function ElementUnitSequence:on_executed(...)
	...
	if self._values.enabled and Global.game_settings and not Network:is_client() then
		...
		--Big Bank
		if level_id == "big" and RevertTitanCams._settings.rtc_big then
			--106265 func_sequence_013
			if self._id == 106265 then
				return
			end
		--Four Stores
		elseif level_id == "four_stores" and RevertTitanCams._settings.rtc_four_stores then
			--103683 seq_deathwish_camera
			if self._id == 103683 then
				return
			end
		--Big Oil Day 1 (Day/Night)
		elseif (level_id == "welcome_to_the_jungle_1" or level_id == "welcome_to_the_jungle_1_night") and RevertTitanCams._settings.rtc_welcome_to_the_jungle_1 then
			--101301 seq_deathwish_cameras
			if self._id == 101301 then
				return
			end
		--Diamond Store
		elseif level_id == "family" and RevertTitanCams._settings.rtc_family then
